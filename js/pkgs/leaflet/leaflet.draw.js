...
L.Polyline.Draw = L.Handler.Draw.extend({
	...
	undo: function () {
		if (this._poly.getLatLngs().length > 1) {
			...
			this._poly.spliceLatLngs(this._poly.getLatLngs().length-1,1);
			this._map.addLayer(this._poly);
		}
	},
...
L.Polygon.Draw = L.Polyline.Draw.extend({
	...
	_updateMarkerHandler: function () {
		...
		if (this._markers.length === 1) {
			this._markers[0].on('click', this._finishShapep, this);
		}
	},
	...
	_cleanUpShape: function () {
		if (this._markers.length > 0) {
			this._markers[0].off('click', this._finishShapep);
			$('#dialog_undo').dialog("close");
		}
	}
});
...
L.Control.Draw = L.Control.extend({
	...
	_createButton: function (title, className, container, fn, context) {
		...
		L.DomEvent
			...
			.addListener(link, 'click', kaushik);
		return link;
	},
